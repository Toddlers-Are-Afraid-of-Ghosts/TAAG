...
public class GenerationV3 : MonoBehaviour {
    public static RoomsProperties[,] Spawn(int size) {
        ...
        var special = PlaceDeadEnds(grid, size);
    ...
    public static RoomsProperties[,] CreateGrid(int size) {
        var grid = new RoomsProperties[size, size];
    ...
    public static void PlacePath(RoomsProperties[,] grid, int size, int[][] specialRooms) {
        var rng = new Random();
        var middle = size / 2;
        foreach (var special in specialRooms) {
            var x = special[0];
            var y = special[1];
    ...
    public static void PlaceSpecial(RoomsProperties[,] grid, int[][] special) {
        ...
        foreach (var spe in special) list.Add(spe);
    ...
    public static void ChooseRoom(RoomsProperties[,] grid, int x, int y, List<RoomsProperties>[] roomArray) {
        if (grid[x, y].IsSpecial) {
            if (grid[x, y].Top) {
                ...
                else if (grid[x, y].IsDeadEnd) grid[x, y].Room = roomArray[15][3].Room;
            }
                ...
                else if (grid[x, y].IsDeadEnd) grid[x, y].Room = roomArray[16][3].Room;
            }
                ...
                else if (grid[x, y].IsDeadEnd) grid[x, y].Room = roomArray[17][3].Room;
            }
                ...
                else if (grid[x, y].IsDeadEnd) grid[x, y].Room = roomArray[18][3].Room;
            }
        }
